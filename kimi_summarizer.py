...
def translate(content):
    if not KIMI_API_KEY:
        logging.error("kimi_API_KEY 未设置")
        sys.exit(1)
    ...
    for attempt in range(5):
        try:
            logging.info(f"翻译请求 (尝试 {attempt + 1}/5)...")
...
def translate_file(filepath):
    ...
    logging.info(f"开始翻译: {filepath} ({len(content)} 字符)")
    result = translate(content)

    if result:
        with open(outpath, 'w', encoding='utf-8') as f:
            f.write(result)
        logging.info(f"翻译完成: {outpath}")
        return True
    else:
        logging.error("翻译失败")
        return False
